...
Data sourcing is achieved through `get_geography_data()` function. Following from the example below the function can be applied as follows
...
For the efficiency the objects `data("available_data_sets")` and `data("standard_geography_code_register")` are provided in the package. Those objects are not updated too frequently and providing them as statics data sets makes sense from the efficiency perspective. It is however possible to source live the mentioned objects. This is discussed in `vignette("support-functions")`.
